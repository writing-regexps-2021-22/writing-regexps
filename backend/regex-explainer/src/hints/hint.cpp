...
Hint get_hint(const UnexpectedEnd& error) {
    std::string hint = "Unexpected end of input at position " + std::to_string(error.position())
        + ": expected " + error.expected();

    std::string additional_info;
    if (error.needs_closing() == U')') {
...
Hint get_hint(const ExpectedEnd& error) {
    std::string hint = "Expected the input to end at position " + std::to_string(error.position())
        + ", but got the character " + wr22::unicode::to_utf8(error.char_got());

    return Hint{hint};
}

Hint get_hint(const UnexpectedChar& error) {
    std::string hint = "Expected " + error.expected() + ", but got the character "
        + wr22::unicode::to_utf8(error.char_got()) + " at position "
...
Hint get_hint(const InvalidRange& error) {
    std::string hint = "Invalid character range " + wr22::unicode::to_utf8(error.first()) + "–"
        + wr22::unicode::to_utf8(error.last()) + " at positions from "
        + std::to_string(error.span().begin()) + " to " + std::to_string(error.span().end());
