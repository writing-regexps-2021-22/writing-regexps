...
namespace wr22::regex_explainer::explanation {
    ...
    ///
    /// If the sample consists of one part, then simply returns std::string_view.
    /// If the sample consists of several parts, then a vector of patterns with type std::string_view is returned,
    /// which should then make up a whole sentence / paragraph.

    std::string_view get_sample(const Empty &vertex) {
        std::string_view pattern1 = "Is empty";
        return pattern1;
    }
    ...
    std::string_view get_sample(const Alternatives &vertex) {
        std::string_view pattern1 = "Alternative";
        return pattern1;
    }
    ...
    std::string_view get_sample(const Group &vertex) {
        std::string_view pattern1 = "Capturing Group";
        return pattern1;
    }

    std::string_view get_sample(const Optional &vertex) {
        std::string_view pattern1 = "matches the previous token between zero and one times, "
                                    "as many times as possible, giving back as needed (greedy)";
        return pattern1;
    }

    std::string_view get_sample(const Plus &vertex) {
        std::string_view pattern1 = "matches the previous token between one and unlimited times, "
                                    "as many times as possible, giving back as needed (greedy)";
        return pattern1;
    }

    std::string_view get_sample(const Star &vertex) {
        std::string_view pattern1 = "matches the previous token between zero and unlimited times, "
                                    "as many times as possible, giving back as needed (greedy)";
        return pattern1;
    }

    std::string_view get_sample(const Wildcard &vertex) {
        std::string_view pattern1 = "matches any character (except for line terminators)";
        return pattern1;
    }

    std::vector<std::string_view> get_sample(const CharacterClass &vertex) {
        std::string_view main_pattern = "Match a single character present in the list.",
